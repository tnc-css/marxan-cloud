...
seed-geoapi-init-data:
	...
	@for i in api/apps/api/test/fixtures/features/*.sql; do \
		...
		cat api/apps/api/test/fixtures/features/$${table_name}.sql | docker-compose $(DOCKER_COMPOSE_FILE) exec -T $(GEO_DB_INSTANCE) sh -c featureid=$$featureid 'sed -e "s/\$$feature_id/\$$featureid/g" | psql -U "${GEO_POSTGRES_USER}"'; \
		done;
